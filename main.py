...
# --- перевод
from smart_translate import translate_text
...
class ChatRequest(BaseModel):
    ...
    max_tokens: int = 100
    lang: Optional[str] = "RU"   # --- перевод: добавлен язык
...
def chat(request: ChatRequest):
    try:
        ...
        user_lang = request.lang.upper() if request.lang else "RU"   # --- перевод

        # --- перевод: если язык не русский, переводим на русский
        if user_lang != "RU":
            try:
                translated_question = translate_text(request.question, user_lang, "RU")
            except Exception as e:
                logger.error(f"Ошибка перевода вопроса: {e}")
                raise HTTPException(status_code=502, detail="Ошибка перевода вопроса")
        else:
            translated_question = request.question

        # Получаем ответ на русском
        answer, confidence, gen_time, source = chatbot.get_smart_answer(
            translated_question,
            request.max_tokens
        )

        # --- перевод: если язык не русский, переводим ответ обратно
        if user_lang != "RU":
            try:
                translated_answer = translate_text(answer, "RU", user_lang)
            except Exception as e:
                logger.error(f"Ошибка перевода ответа: {e}")
                raise HTTPException(status_code=502, detail="Ошибка перевода ответа")
        else:
